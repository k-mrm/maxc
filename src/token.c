...
enum TKIND tk_char2(int c1, int c2) {
    ...
    case '=':
        switch(c2) {
        case '=': return TKIND_Eq;
        }
    case '<':
        switch(c2) {
        case '=': return TKIND_Lte;
        }
    case '>':
        switch(c2) {
        case '=': return TKIND_Gte;
        }
    case '!':
        switch(c2) {
        case '=': return TKIND_Neq;
        }
    case '+':
        switch(c2) {
    ...
    case '*':
        switch(c2) {
        case '=': return TKIND_AsteriskAs;
        }
    case '/':
        switch(c2) {
        case '=': return TKIND_DivAs;
        }
    case '%':
        switch(c2) {
        case '=': return TKIND_ModAs;
        }
    default:
...
void setup_token() {
    ...
    for(int i = 0; i < kwmap_len; i++) {
        map_push(keywordmap, (void *)kwmap[i].key, (void *)(intptr_t)kwmap[i].kind);
    }
}

static Token *New_Token(enum TKIND kind, String *value, Location s, Location e) {
...
static Token *New_Token_With_Symbol(enum TKIND kind, uint8_t len, Location s, Location e) {
    Token *self = malloc(sizeof(Token));
...
void token_push_symbol(Vector *self, enum TKIND kind, uint8_t len, Location s, Location e) {
